...
export default function LoginPage({ 
  ...
  const handleDemoLogin = async () => {
    ...
    try {
      ...
      setGeneralError('Demo login unavailable. Please try standard sign in.');
    } finally {
  ...
  const handleGoogleSignIn = async () => {
    ...
    if (!isFirebaseConfigured()) {
      const msg = 'Firebase is not yet configured. Please configure your VITE_FIREBASE_* keys in client/.env.';
      setGeneralError(msg);
      showToast({
        type: 'warning',
        message: 'Firebase configuration required. Please check client/.env'
      });
      return;
    }
    ...
    try {
      ...
      showToast({
        type: 'success',
        message: `Welcome${googleResult.displayName ? `, ${googleResult.displayName}` : ''}!`
      });
      ...
      const friendlyMsg = mapFirebaseAuthError(err) || err.response?.data?.detail || 'Google sign-in failed.';
  ...
  return (
    <div className="relative min-h-[calc(100vh-4rem)] pt-24 pb-16 flex items-center justify-center px-4 sm:px-6 lg:px-8">
      ...
      <div className="w-full max-w-5xl grid grid-cols-1 lg:grid-cols-12 gap-8 items-center relative z-10">
        ...
        {/* Left Side: Editorial & Biotech DNA Context */}
        <div className="lg:col-span-6 text-left space-y-6 hidden lg:block pr-6">
          ...
          <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight text-white leading-tight">
            Decode your health reports with{' '}
          ...
          <p className="text-slate-400 text-sm leading-relaxed max-w-md">
            MediLens transforms complex laboratory values, metabolic markers, and clinical diagnostics into plain-language summaries and actionable health insights.
          </p>
          ...
          <div className="pt-2 space-y-3">
            <div className="flex items-center gap-3 text-xs text-slate-300">
              ...
              <span>Private & secure health records</span>
            </div>
            <div className="flex items-center gap-3 text-xs text-slate-300">
              <div className="w-5 h-5 rounded-full bg-orange-500/20 border border-orange-500/30 flex items-center justify-center text-orange-400">
                ✓
              </div>
              <span>Continuous 3D biometric tracking & trend analysis</span>
            </div>
            <div className="flex items-center gap-3 text-xs text-slate-300">
              <div className="w-5 h-5 rounded-full bg-purple-500/20 border border-purple-500/30 flex items-center justify-center text-purple-400">
                ✓
              </div>
              <span>Conversational AI medical report assistant</span>
            </div>
          </div>
        </div>
          ...
          <div className="relative rounded-3xl p-6 sm:p-8 bg-[#0d0f15]/85 border border-white/10 backdrop-blur-2xl shadow-2xl shadow-black/60 text-left transition-all">
            ...
            <div className="text-center mb-6">
              ...
              <p className="text-xs text-slate-400 mt-1.5 font-medium">
                Sign in to continue to MediLens.
              </p>
            </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <button
                    ...
                    className="text-[11px] font-semibold text-orange-400 hover:text-orange-300 transition-colors cursor-pointer"
                  >
                    Forgot Password?
                  </button>
                </div>
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full pl-10 pr-11 py-3 rounded-xl bg-slate-900/90 border text-xs text-white placeholder-slate-500 focus:outline-none transition-all ${
            ...
            <button
              ...
              className="w-full py-2.5 px-4 rounded-xl text-xs font-semibold text-slate-200 bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 transition-all flex items-center justify-center gap-2.5 cursor-pointer transform hover:-translate-y-0.5 active:scale-95 disabled:opacity-50"
            >
              {googleLoading ? (
                <>
                  <span className="w-3.5 h-3.5 border-2 border-white/40 border-t-white rounded-full animate-spin" />
            ...
            {/* Instant Demo Account (Extra convenience for testing) */}
            <div className="mt-3">
              <button
                type="button"
                onClick={handleDemoLogin}
                disabled={loading || demoLoading}
                className="w-full py-2 px-3 rounded-xl text-[11px] font-semibold text-amber-300/90 bg-amber-500/10 hover:bg-amber-500/15 border border-amber-500/20 transition-all flex items-center justify-center gap-1.5 cursor-pointer"
              >
                <Zap className="w-3.5 h-3.5 text-amber-400" />
                <span>{demoLoading ? 'Connecting...' : 'Instant 1-Click Demo Sign In'}</span>
              </button>
            </div>
